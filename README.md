...
This project takes the opposite approach. The entire CLI is ~1,100 lines of Go. The Unity-side connector is ~1,800 lines of C#. There is no protocol layer, no relay process, no virtual environment — just an HTTP POST from a binary to Unity's built-in HttpListener.
